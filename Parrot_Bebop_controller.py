...
class PID():
    def __init__(self, Kp, Kd, Ki, T, max_val, min_value):
        ...
        # varijable u koje spremamo vrijednost greske iz prethodnog koraka
    ...
    # funkcija za update upravljackih vrijednosti
...
class Hover():
    ...
    def __init__(self):
        ...
        # unose se vrijednosti kontrolera dobivene u Simulinku za pojedine linearizirane modele
    ...
    def slika_callback(self,data):
        # dobivanje koordinata centra snimke kamere
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # ako vidi QR updatea upravljacke vrijednosti 
            if self.flag1:
            ...
            # ako nije pritisnut gumb na joysticku automatski nacin rada
            if not self.button:
                self.nacin_rada.publish("automatski nacin rada")
            else:
                self.nacin_rada.publish("rucni nacin rada")

            # nastimaj smjerove brzina da se slazu
            self.vel.linear.y = -ux
            self.vel.linear.x = uy
            
            # pokreni sphero robote ako je sve u redu
